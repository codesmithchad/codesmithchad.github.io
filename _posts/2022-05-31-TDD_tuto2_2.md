...
[previous post]([to be continued..](https://codesmithchad.github.io/2022-05-31-TDD_tuto2_1/)
...
\
테스트 페인(cmd+6)으로 이동하여 +를 누르고 `New Unit Test Class`를 추가하고 `BullsEyeFakeTests`로 명명하고 `BullsEyeTests` 디렉토리에 저장하여 BullsEyeTests의 타겟으로 설정한다.\
\
...
`URLSessionProtocol` 프로토콜을 따르는`URLSessionStub` 정의되어 있으며 이니셜라이저가 데이터, reponse, 에러 데이터를 리턴하도록 정의되어 있다.\
\
...
앱 화면 우하단에 있는 세그먼트 컨트롤을 통해 게임 스타일을 변경하고 이것을 UserDefatuls에 저장한다.\
\
...
assertion 타이밍은 테스트 메소드가 세그먼트 컨트롤을 변경하여 `gameStyleChanged` 플래그는 0이 될 때이다.\
assertion이 true라면 `set(_:forKey:)`가 한번 호출될 것이다.\
...
`testGameStyleSwitch()`에 새 라인을 하나 추가하고 디버그 콘솔 상단에 있는 녹화 버튼을 누른다.\
이는 시뮬레이터를 녹화모드로 앱을 실행하게하고 인터렉션을 테스트 커맨드로 캡쳐한다.\
앱이 로드되면 게임 스타일 스위치에 있는 슬라이드 세그먼트와 상단 레이블을 탭하자.\
녹화 버튼을 다시 눌러 녹화를 중단하자.\
\
그러면 아래와 같은 3개의 라인이 생긴다.
...
애플의 공식 문서에 따르면 퍼포먼스 테스트의 의미는 아래와 같다.
...
측정하고 하는 곳에 measure() 클로저를 추가하자.\
추가로 몇번 측정할 것인지도 지정할 수 있다.
